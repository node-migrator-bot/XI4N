...
var cli = function()
{
	...
	self.execute = function()
	{
		...
		if (self.program.update)
			...
			{
				self.updatePlugins(self.program.config, function()
				{
					process.exit(0);
				});
			});
		...
		if (self.program.install)
			...
			{
				self.updatePlugins(self.program.install, function()
				{
					process.exit(0);
				});
			});
	...
	self.updatePlugins = function(config, next)
	{
		...
		for (var f in files)
		{
			var stat = fs.lstatSync(path.join(config, f));
		    if (!stat.isDirectory())
				continue;

			pkgs.push(path.join(config, f));
		}
		...
		npm.load(function (err)
		{
			...
			npm.commands.update(plugins, function(err, data)
			{
				next();
			});
		});
